FROM mongo:3.3.6

MAINTAINER Vincenzo FERME <info@vincenzoferme.it>

## copy the custom data and scripts
COPY data/seed/icwe2016dockertutorial /data/db/icwe2016dockertutorial
COPY bin/initilize-mongo.sh /initilize-mongo.sh 
## add execution priviledges to initilize-mongo.sh
RUN chmod +x /initilize-mongo.sh 

## ovveride the entrypoint, and consequently the command
ENTRYPOINT ["/initilize-mongo.sh"]

CMD ["mongod"]